...
class TestDeskAssetRevision(unittest.TestCase):
	def test_desk_hooks_use_revisioned_entry_points(self):
		...
		self.assertIn("/assets/gg_power/css/gg_power_desk_v9.css", hooks)
		self.assertIn("/assets/gg_power/js/gg_power_desk_v9.js", hooks)

	def test_revisioned_assets_force_fresh_base_assets(self):
		css_entry = (APP_ROOT / "gg_power" / "public" / "css" / "gg_power_desk_v9.css").read_text(
			encoding="utf-8"
		)
		js_entry = (APP_ROOT / "gg_power" / "public" / "js" / "gg_power_desk_v9.js").read_text(
			encoding="utf-8"
		)

		self.assertIn("gg_power_desk.css?revision=", css_entry)
		self.assertIn("gg_power_manufacturing_dashboard.css?revision=", css_entry)
		self.assertIn("gg_power_desk.js?revision=", js_entry)
		self.assertIn("gg_power_manufacturing_dashboard.js?revision=", js_entry)
